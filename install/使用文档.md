...
#### 问题反馈和交流
大家有问题直接建Issues即可，这样也方便别人看到，也不会被刷屏，也可以看wiki常见问题。
...
1. **Linux** : 下载HotSecondsServer.zip到本地解压，然后去[libjvm.so](https://github.com/thanple/HotSecondsIDEA/blob/master/install/libjvm_so.md)找对应的jdk版本下载完后无需改名直接放到解压后目录的./lib下。<br>
  &nbsp;&nbsp;再然后上传到服务器(记目录为path1)，执行sh install.sh. <br>
  **Windows & MacOS 本地热部署** :  先去[dcevm](https://github.com/thanple/HotSecondsIDEA/blob/master/install/dcevm_installer.md) 找到你的JDK版本的jar包，下载下来后运行jar（可能需要管理员权限或者root权限）安装，选择Install DCEVM as altjvm，然后将安装包解压后，放到你的工程根目录，记目录为$path1.
<br>
...
2.将hot-seconds-remote.xml复制到代码的resource目录，根据需要修改secret和classloader等配置<br><br>
...
<br><br>
3.添加jvm参数开启热部署代理 -XXaltjvm=dcevm -javaagent:$path1/HotSecondsServer.jar=hotconf=$path2/hot-seconds-remote.xml<br>
...
1.首先禁用IDEA自带的HotSwap功能，设置->Build,Execution,Deployment->Debugger->HotSwap->Reload classes after compilation 选 Never，这样就不会和IDEA自带的热更新冲突.
![](https://github.com/thanple/HotSecondsIDEA/blob/master/img/cancel_ori_hotswap.png)

<br><br>

然后在插件市场下载安装HotSecondsClient<br>
...
- 打开java文件，右键选择Hot swap this file to remote可热部署当前文件，也可以在资源文件上右键热部署，但是资源文件需要在配置里面配置路径映射.<br>
...
### [点击这里-保姆级介绍](https://github.com/thanple/HotSecondsIDEA/wiki/HotSeconds%E6%89%80%E6%9C%89%E5%8A%9F%E8%83%BD%E4%BB%8B%E7%BB%8D)
